...
var g = {
    ...
    filter: { equalization: !0 },
    face: {
      enabled: !0,
      detector: { rotation: !0, return: !0, cropFactor: 1.6, mask: !1 },
  ...
  d = {
    ...
    cropFactor: g.face.detector.cropFactor,
    ...B,
  },
  e = {
...
async function R() {
  ...
  if (
    ...
    (i.includes("blink left eye") || i.includes("blink right eye")) &&
      ...
      (e.blinkDetected.status =
        e.blinkDetected.status ||
        (Math.abs(u.end - u.start) > d.blinkMin &&
          Math.abs(u.end - u.start) < d.blinkMax)),
      e.blinkDetected.status &&
        u.time === 0 &&
      ...
      (e.faceConfidence.status = e.faceConfidence.val >= d.minConfidence),
      (e.antispoofCheck.val = s.result.face[0].real || 0),
      (e.antispoofCheck.status = e.antispoofCheck.val >= d.minConfidence),
      (e.livenessCheck.val = s.result.face[0].live || 0),
      (e.livenessCheck.status = e.livenessCheck.val >= d.minConfidence),
      (e.faceSize.val = Math.min(
        s.result.face[0].box[2],
        s.result.face[0].box[3]
      )),
      (e.faceSize.status = e.faceSize.val >= d.minSize),
      (e.descriptor.val =
      ...
      (e.gender.status = e.gender.val >= d.minConfidence);
  }
  return (
    (e.timeout.status = e.elapsedMs.val <= d.maxTime),
    P(),
    E() || !e.timeout.status
...
async function $() {
  ...
  if (
    ...
    (i = t.canvas.getContext("2d")) == null ||
      i.clearRect(0, 0, d.minSize, d.minSize),
  ...
  if (
    ...
    r(
      `detected face: ${c.face.gender} ${c.face.age || 0}y distance ${
        c.face.iris || 0
      }cm/${Math.round((100 * (c.face.iris || 0)) / 2.54) / 100}in`
    ),
    await s.tf.browser.toPixels(c.face.tensor, t.canvas),
    (await b()) === 0)
  )
    return (
      r("face database is empty: nothing to compare face with"),
  ...
  let a = await C(),
    ...
    o = s.match(c.face.embedding, n, B);
  return (
    ...
    (document.body.style.background =
      o.similarity > d.threshold ? "darkgreen" : "maroon"),
...
async function M() {
  ...
  return (
    ...
    (c.face = await R()),
    (t.canvas.width =
      ((a = c.face.tensor) == null ? void 0 : a.shape[1]) || d.minSize),
    (t.canvas.height =
      ((n = c.face.tensor) == null ? void 0 : n.shape[0]) || d.minSize),
    ...
    E() ? $() : (r("did not find valid face"), !1)
  );
}
async function q() {
  var a, n;
  r("human version:", s.version, "| tfjs version:", s.tf.version["tfjs-core"]),
    r(
      "options:",
      JSON.stringify(d)
        .replace(/{|}|"|\[|\]/g, "")
    ...
    r("initializing webcam..."),
    await H(),
    r("loading human models..."),
    await s.load(),
    r("initializing human..."),
    r(
    ...
    r("loading face database..."),
    r("known face records:", await b()),
    t.retry.addEventListener("click", M),
    t.save.addEventListener("click", z),
